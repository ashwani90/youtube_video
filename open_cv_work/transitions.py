...
def add_speech_bubble(img, text, position=(50, 50), bubble_size=(250, 120), fade_in_frames=30):
    ...
    # Add text
    cv2.putText(frame, text, (x + 15, y + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
    return frame

def transition_to_cartoon(image_path, text, style="cartoon", transition_frames=50):
    """Applies a smooth transition from original image to a cartoon effect with a speech bubble."""
    ...
    final_frame = add_speech_bubble(cartoon, text)
